  ...
  try {
    ...
    const [payments] = await db.query(
      `SELECT 
        ...
        o.query_code, o.paper_topic as topic, o.total_price_usd,
  ...
  try {
    ...
    const [[payment]] = await db.query(
      `SELECT 
        ...
        o.order_id as order_full, o.query_code, o.paper_topic as topic, o.total_price_usd, o.status as order_status,
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    const [[payment]] = await db.query(
      `SELECT 
        p.payment_id, p.order_id, p.user_id, p.amount, p.payment_method,
        u.full_name, u.email, o.order_id as order_exists, o.total_price_usd
      FROM payments p
      JOIN users u ON p.user_id = u.user_id
      LEFT JOIN orders o ON p.order_id = o.order_id
      WHERE p.payment_id = ?`,
    ...
    if (verificationStatus === 'verified') {
      // Generate work_code if not exists
      const workCode = `WC${Date.now()}${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
      ...
      await logAction({
        ...
        details: `Payment verified. Amount: $${payment.amount}. Work Code: ${workCode}`,
        resource_type: 'order',
      ...
      sendMail({
        ...
        html: `
          ...
          <p>Your payment of $${payment.amount} has been verified successfully.</p>
          <p>Your work code: <strong>${workCode}</strong></p>
