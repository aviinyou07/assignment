...
exports.createQuery = async (req, res) => {
  ...
  try {
    ...
    // =======================
    // GENERATE QUERY CODE (Format: YYYY + FN + LN + MM + OrderNo)
    // Example: 2025SS1001
    // =======================
    const query_code = await generateQueryCode(user.full_name);
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NULL, NOW())`,
    ...
    // Quotation is generated by Admin/BDE - NOT auto-created
    // Status will change to 27 when Admin sends quotation
    // ============================================================================
...
exports.trackOrderByWorkCode = async (req, res) => {
  try {
    ...
    // =======================
    // TRANSLATE STATUS TO CLIENT VIEW
    // Clients see simplified statuses only
    // =======================
    const { CLIENT_VISIBLE_STATUSES } = require('../utils/state-machine');
    const clientStatusName = CLIENT_VISIBLE_STATUSES[order.status] || order.status_name;
    ...
    return res.json({
      ...
      data: {
        order: {
          ...order,
          client_status: clientStatusName,  // Simplified status for client
